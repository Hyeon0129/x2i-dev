...
date: "2025-03-10"
category: "Guides"
slug: "tenstorrent_npu_setup_guide"
thumbnail: "/images/thumbnails/guides-tenstorrent-npu-setup-guide-install-drivers-and-sdk-thumbnail.png"
description: "Complete step-by-step guide to install Tenstorrent Wormhole N300S NPU drivers (TT-KMD), firmware (TT-Flash), and system tools on Ubuntu 22.04. Includes multi-card mesh topology configuration."
keywords: [Tenstorrent, Wormhole N300S, NPU setup, TT-KMD driver, TT-Flash firmware, TT-SMI, TT-Topology, Ubuntu 22.04, AI accelerator]
...
In this guide, we will walk you through the setup process for your **Tenstorrent NPU wormhole (N300S)**, including hardware installation, driver setup
...
Installing the Kernel-Mode Driver (TT-KMD) is an essential step for setting up the **Tenstorrent Wormhole N300S** system correctly.
...
### 5. Install the System Management Interface TT-SMI
...
You will see a display with device information, telemetry, and firmware:
![tt-smi](/images/posts/guides/tenstorrent_npu_setup_guide/tt-smi.png)

<br />

You can check the results of the firmware update to 80.15.0.0 applied previously.
![tt-smi-fimrware-update](/images/posts/guides/tenstorrent_npu_setup_guide/tt-smi-firmware-update.png)

<br />

### 6. Optional : Multi-Card Configuration TT-Topology
...
[TT-Topology](https://github.com/tenstorrent/tt-topology) is a command line utility used to flash multiple NB cards on a system to use specific eth routing configurations.
...
#### Mesh
In the mesh layout is a trivalent graph where each node can have a max of 3 connection. The system assigns the coordinates using a BFS algorithm. Command to generate a mesh layout

![tt-topology-mesh](/images/posts/guides/tenstorrent_npu_setup_guide/tt-topology-mesh.png)
...
![tt-smi-topology](/images/posts/guides/tenstorrent_npu_setup_guide/tt-smi-topology.png)

<br />

Now, all the basic configurations are complete.
Tenstorrent provides three open-source SDKs for developing on Tensix Processors:

- TT-Buda to run existing models
...
As of October 2024, TT-Buda development has been discontinued. It is planned to be integrated with other SDKs in the future.
TT-Metalium/TT-NN and TT-Forge/TT-MLIR are currently under active development.
In the next section, we will document the installation of the SDK and run example demos. N300S
